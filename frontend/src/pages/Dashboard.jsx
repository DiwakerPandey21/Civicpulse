...
import { FaMapMarkerAlt, FaCamera, FaStar, FaCommentDots, FaClipboardCheck, FaTrashAlt, FaBolt, FaWater, FaExclamationTriangle, FaBullhorn, FaChartLine } from 'react-icons/fa';
            ...
            <div className="pt-20 pb-24 px-4 sm:px-6">
                ...
                <div className="max-w-4xl mx-auto mb-8 text-center">
                    ...
                    <h1 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-saffron-600 to-indiaGreen-600 dark:from-saffron-400 dark:to-indiaGreen-400 mb-2">
                        Welcome, {user.role === 'official' ? 'Officer' : 'Swachhata Citizen'} {user.name}
                    </h1>
                    {user.role === 'official' && user.department && user.department !== 'None' && (
                        <span className="inline-block mb-2 px-3 py-1 bg-blue-100 text-blue-800 text-xs font-bold rounded-full dark:bg-blue-900 dark:text-blue-200 border border-blue-200 dark:border-blue-800">
                            Department: {user.department}
                        </span>
                    )}
                    <p className="text-slate-600 dark:text-slate-400">
                        {user.role === 'official' ? 'Here are the assigned tasks for your department.' : "Here are today's actions for you. Let's keep India clean! 🇮🇳"}
                    </p>
                </div>
                ...
                {(user.role === 'admin' || user.role === 'official') && (
                    <div className="max-w-7xl mx-auto mb-10 space-y-6">
                        {/* Broadcast Alert Widget (Admin Only) */}
                        {user.role === 'admin' && (
                            <div className="bg-gradient-to-br from-red-50 to-orange-50 dark:from-red-900/20 dark:to-orange-900/20 p-6 rounded-2xl shadow-lg border border-red-100 dark:border-red-800">
                                <h3 className="font-bold text-red-800 dark:text-red-300 mb-4 flex items-center text-lg">
                                    <FaBullhorn className="mr-3" /> Broadcast Emergency Alert
                                </h3>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <input type="text" id="alertTitle" placeholder="Alert Title (e.g., Heavy Rainfall Warning)" className="w-full p-3 rounded-xl border border-red-200 dark:border-red-900 bg-white dark:bg-slate-800 text-sm focus:ring-2 focus:ring-red-500 outline-none" />
                                    <select id="alertType" className="p-3 rounded-xl border border-red-200 dark:border-red-900 bg-white dark:bg-slate-800 text-sm focus:ring-2 focus:ring-red-500 outline-none">
                                ...
                                <textarea id="alertMessage" placeholder="Detailed message for citizens..." className="w-full mt-4 p-3 rounded-xl border border-red-200 dark:border-red-900 bg-white dark:bg-slate-800 text-sm focus:ring-2 focus:ring-red-500 outline-none" rows="2"></textarea>
                                    ...
                                    <button
                                        onClick={async () => {
                                            ...
                                            try {
                                                ...
                                                alert("Failed to broadcast alert: " + (e.response?.data?.message || e.message));
                                            }
                                        }}
                                        className="bg-red-600 hover:bg-red-700 text-white px-6 py-2.5 rounded-xl font-bold text-sm shadow-md transition-transform active:scale-95 flex items-center"
                                    >
                ...
                {/* Main Action Tiles (Grid Layout like Swachhata App) - ONLY FOR CITIZENS */}
                {user.role !== 'admin' && user.role !== 'official' && (
                ...
                <div className="max-w-7xl mx-auto">
                    <h2 className="text-xl font-bold text-slate-800 dark:text-white mb-4 flex items-center">
                        <FaClipboardCheck className="mr-2 text-saffron-600" />
                        {user.role === 'official' ? 'Pending Department Tasks' : 'Your Activity'}
                    </h2>
                        ...
                        {complaints.length > 0 ? (
                            ...
                            <div className="text-center py-10 bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700">
                                <p className="text-slate-500 dark:text-slate-400">
                                    {user.role === 'official' ? 'No pending complaints for your department.' : 'No complaints reported yet.'}
                                </p>
                                {user.role !== 'official' && (
